"""Compile the first complete five-seed ICCE T/T+S validation contrast.

The full manuscript artifact compiler intentionally requires all 72 registered
validation cells.  This smaller compiler permits only the ten already-terminal
100%-label T and regime-separated T+S cells.  It calls the same public analysis
kernel and bootstrap seeds as the eventual six-method attribution analysis.
It accepts no locked-test or processed-extra path and does not authorize a
paper claim.
"""
...
from aurora.aneug_release_730_icce_analysis import analyze_primary_pair
...
INPUT_SCHEMA = "aurora.private.aneug_release_730_icce_primary_pair_inputs.v1"
OUTPUT_SCHEMA = "aurora.aneug_release_730_icce_primary_pair_bundle.v1"
...
class ICCEPrimaryPairError(RuntimeError):
    """Raised when primary-pair evidence is incomplete or not traceable."""


def _require(condition: bool, reason: str) -> None:
    if not condition:
        raise ICCEPrimaryPairError(reason)
...
def compile_primary_pair(
    *,
    input_manifest_path: Path,
    output_path: Path,
) -> dict[str, Any]:
    """Validate ten terminal cells and atomically emit their paired analysis."""

    _require(not output_path.exists(), "output_exists")
    manifest = _load(input_manifest_path)
    _require(
        manifest.get("schema_version") == INPUT_SCHEMA
        and manifest.get("status") == "complete_ten_terminal_validation_cells"
        and manifest.get("protocol_id") == PROTOCOL_ID
        and manifest.get("methods") == list(METHODS)
    ...
    _require(isinstance(entries, list) and len(entries) == 10, "manifest_cells")
    ...
    for entry in entries:
        ...
        results[method][int(training_seed)] = result
    ...
    analysis = analyze_primary_pair(results, protocol)
    output = {
