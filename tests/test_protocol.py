...
from aurora.aneux_nested_orbit_p0_v2 import (
    ...
    load_config as load_nested_orbit_config,
    validate_config as validate_nested_orbit_config,
)
...
class ProtocolTests(unittest.TestCase):
    ...
    def test_response_fidelity_is_the_only_conditional_lead_and_p0_is_closed(self) -> None:
        ...
        self.assertFalse(anchor_audit["v2_executed"])
        self.assertFalse(anchor_audit["v2_cache_or_field_read"])
        self.assertTrue(anchor_audit["v2_anchor_tangent_omitted"])
        self.assertTrue(anchor_audit["synthetic_v2_all_checks_passed"])
        self.assertTrue(anchor_audit["synthetic_v3_anchor_check_failed"])
        self.assertTrue(
            anchor_audit["synthetic_negative_control_is_not_scientific_evidence"]
        )
        self.assertEqual(anchor_audit["v2_registered_check_count"], 11)
        ...
        self.assertTrue(anchor_audit["exact_cache_path_publicly_redacted"])
        self.assertEqual(
            anchor_audit["exact_cache_sha256"],
        ...
        self.assertFalse(anchor_audit["real_cache_field_array_read"])
        self.assertFalse(anchor_audit["historical_p1_v1_v2_v3_activatable"])
        self.assertTrue(
        ...
        candidate["problem_selection"][
            "aneumo_response_fidelity_p0_anchor_tangent_red_team"
        ]["v3_observed_check_count"] = 1
        with self.assertRaisesRegex(ProtocolError, "anchor-tangent"):
            validate_protocol(candidate)
    ...
    def test_open_clinical_table_is_versioned_but_not_active_imaging_data(self) -> None:
        problem = self.protocol["problem_selection"]
        audit = problem["open_clinical_outcome_and_target_time_reappraisal"]
        watch = problem["public_source_watch_v21"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(audit["best_score"], 29.5)
        self.assertEqual(audit["asah_risk_zenodo_record_id"], 17339029)
        self.assertEqual(audit["asah_risk_file_bytes"], 39686)
        self.assertEqual(audit["asah_risk_source_patients"], 230)
        self.assertEqual(audit["asah_risk_missing_six_month_mrs_patients"], 70)
        self.assertFalse(audit["asah_risk_fixed_six_month_outcome_identified"])
        self.assertFalse(audit["asah_risk_medical_image_payload_present"])
        self.assertFalse(audit["asah_risk_xlsx_payload_downloaded_or_opened"])
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertFalse(audit["gpu_training_authorized"])
        self.assertEqual(watch["watch_count"], 34)

        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"][
            "open_clinical_outcome_and_target_time_reappraisal"
        ]["asah_risk_medical_image_payload_present"] = True
    ...
    def test_aneug_reference_floor_is_source_only_and_below_admission(self) -> None:
        ...
        self.assertEqual(audit["processed_v4_d1_transient_bytes"], 23_744_862_051)
        self.assertEqual(audit["processed_v4_d1_selected_asset_peak_cap_bytes"], 60_000_000_000)
        self.assertEqual(audit["processed_v4_d1_transport_attempt_limit"], 3)
        self.assertEqual(audit["processed_v4_d1_transport_attempts_used"], 3)
        self.assertEqual(audit["processed_v4_d1_attempt_1_job_id"], "116207.ECE-util1")
        self.assertEqual(audit["processed_v4_d1_attempt_1_partial_bytes"], 0)
        self.assertFalse(audit["processed_v4_d1_attempt_1_schema_evaluated"])
        self.assertEqual(audit["processed_v4_d1_attempt_2_job_id"], "116208.ECE-util1")
        self.assertEqual(audit["processed_v4_d1_attempt_2_partial_bytes"], 0)
        self.assertFalse(audit["processed_v4_d1_attempt_2_schema_evaluated"])
        self.assertEqual(audit["processed_v4_d1_attempt_3_job_id"], "116209.ECE-util1")
        self.assertEqual(audit["processed_v4_d1_attempt_3_partial_bytes"], 0)
        self.assertFalse(audit["processed_v4_d1_attempt_3_schema_evaluated"])
        self.assertTrue(audit["processed_v4_d1_closed"])
        ...
        self.assertEqual(audit["processed_v4_d3_chunks_completed"], 23)
        self.assertEqual(audit["processed_v4_d3_chunk_sftp_sessions_used"], 23)
        self.assertEqual(audit["processed_v4_d3_chunk_resume_sessions_used"], 0)
        self.assertEqual(audit["processed_v4_d3_chunk_hash_mismatches"], 0)
        self.assertEqual(
            audit["processed_v4_d3_server_reassembly_peak_bytes"],
            57_122_234_152,
        )
        self.assertTrue(audit["processed_v4_d3_d2_partial_retired"])
        self.assertEqual(audit["processed_v4_d3_finalizer_attempts_used"], 1)
        self.assertEqual(audit["processed_v4_d3_finalizer_exit_status"], 0)
        self.assertTrue(audit["processed_v4_d3_full_object_exact"])
        self.assertEqual(audit["processed_v4_d3_schema_attempts_used"], 1)
        self.assertEqual(audit["processed_v4_d3_schema_exit_status"], 1)
        self.assertEqual(audit["processed_v4_d3_schema_failure_reason"], "case_floor")
        self.assertEqual(audit["processed_v4_d3_minimum_cases_required"], 700)
        self.assertTrue(audit["processed_v4_d3_registered_case_count_below_minimum"])
        ...
        self.assertEqual(
            audit["processed_v4_d5_status"], "closed_complete_gate_pass"
        )
        self.assertEqual(audit["processed_v4_d5_execution_server"], "introai9")
        self.assertEqual(audit["processed_v4_d5_ngpus"], 0)
        self.assertEqual(audit["processed_v4_d5_attempts_used"], 1)
        self.assertEqual(audit["processed_v4_d5_attempt_limit"], 1)
        self.assertEqual(audit["processed_v4_d5_job_id"], "116483.ECE-util1")
        self.assertEqual((audit["processed_v4_d5_final_state"], audit["processed_v4_d5_exit_status"]), ("F", 0))
        ...
        self.assertEqual(audit["processed_v4_d5_primary_components"], 508)
        self.assertEqual(audit["processed_v4_d5_auxiliary_components"], 68)
        self.assertEqual(audit["processed_v4_d5_train_components"], 406)
        self.assertEqual(audit["processed_v4_d5_validation_components"], 51)
        self.assertEqual(audit["processed_v4_d5_outer_test_components"], 51)
        self.assertTrue(audit["processed_v4_d5_gate_pass"])
        self.assertTrue(audit["processed_v4_d5_private_split_frozen"])
    ...
    def test_mechanistic_treatment_and_growth_batch_is_fail_closed(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        audit = problem["mechanistic_treatment_and_growth_asset_reappraisal"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(gate["current_batch_best_score"], 34.0)
        self.assertEqual(gate["current_batch_best_residual_novelty"], 2.5)
        self.assertEqual(
            audit["best_candidate_id"],
        ...
        self.assertEqual(audit["coil_mechanics_synthetic_sacs"], 500)
        self.assertEqual(audit["qa_occlusion_source_patients"], 458)
        self.assertEqual(audit["qa_occlusion_target_months"], 6)
        self.assertEqual(audit["longitudinal_growth_matched_pairs"], 17)
        self.assertEqual(audit["amplified_wall_motion_growing_cases"], 6)
        self.assertEqual(audit["amplified_wall_motion_stable_cases"], 6)
        self.assertEqual(audit["particle_transport_idealized_anatomies"], 1)
        self.assertFalse(
            audit[
                "same_patient_geometry_device_immediate_response_fixed_time_outcome_join_public"
            ]
        )
        self.assertFalse(audit["supplement_payload_accessed"])
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertTrue(audit["scientific_server_queried"])
        self.assertFalse(audit["gpu_training_authorized"])
        self.assertFalse(audit["junjinyong_accessed"])
        ...
        with self.assertRaisesRegex(ProtocolError, "mechanistic-treatment/growth"):
            validate_protocol(candidate)

    def test_introai9_inventory_is_incomplete_and_cannot_admit_data(self) -> None:
        inventory = self.protocol["problem_selection"][
            "introai9_dataset_inventory_audit"
        ]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(inventory["login_endpoints_attempted"], 2)
        self.assertEqual(inventory["tcp_port_22_reachable_endpoints"], 2)
        self.assertEqual(
            inventory["public_key_authentication_confirmed_endpoints"], 1
        )
        self.assertEqual(inventory["remote_command_output_lines"], 0)
        self.assertFalse(inventory["remote_shell_listing_obtained"])
        self.assertFalse(inventory["sftp_listing_obtained"])
        self.assertFalse(inventory["current_dataset_presence_or_absence_determined"])
        self.assertEqual(inventory["current_direction_verified_train_cases"], 0)
        self.assertEqual(inventory["current_direction_verified_validation_cases"], 0)
        self.assertEqual(inventory["current_direction_verified_test_cases"], 0)
        self.assertFalse(inventory["pbs_job_submitted"])
        self.assertFalse(inventory["gpu_used"])
        self.assertFalse(inventory["junjinyong_accessed"])

        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"]["introai9_dataset_inventory_audit"][
            "current_dataset_presence_or_absence_determined"
        ] = True
        with self.assertRaisesRegex(ProtocolError, "introai9 inventory"):
            validate_protocol(candidate)

    def test_historical_holdings_are_not_relabelled_as_absent_or_active(self) -> None:
        ledger = self.protocol["problem_selection"]["dataset_asset_state_ledger"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(ledger["historical_named_holding_records"], 6)
        self.assertFalse(ledger["historical_holdings_absent_claim"])
        self.assertFalse(ledger["current_introai9_exact_path_inventory_complete"])
        self.assertFalse(ledger["current_introai9_presence_or_absence_verdict"])
        self.assertEqual(ledger["active_dataset_count"], 0)
        self.assertEqual(
            [
                ledger["active_train_cases"],
                ledger["active_validation_cases"],
                ledger["active_test_cases"],
            ],
            [0, 0, 0],
        )
        self.assertEqual(
            {item["id"]: item["scientific_status"] for item in ledger["holdings"]},
            {
        ...
        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"]["dataset_asset_state_ledger"][
            "historical_holdings_absent_claim"
        ] = True
        with self.assertRaisesRegex(ProtocolError, "Historical audited holdings"):
            validate_protocol(candidate)

    def test_aneux_reliability_direction_is_rejected_before_execution(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        historical = problem["acquired_asset_application_direction"]
        audit = problem["aneux_reliability_direct_prior_reappraisal"]
        ...
        self.assertEqual(historical["best_score"], 33.0)
        ...
        self.assertEqual(audit["historical_schema_11_6_score_preserved"], 33.0)
        ...
        self.assertFalse(
            audit["implementation_single_connected_open_surface_gate_present"]
        )
        self.assertFalse(audit["p0_v2_repaired"])
        self.assertFalse(audit["p0_v2_executed"])
        self.assertFalse(audit["p0_v3_created"])
        self.assertFalse(audit["architecture_selected"])
        self.assertFalse(audit["gpu_training_authorized"])
        self.assertFalse(audit["outer_test_authorized"])
        self.assertFalse(audit["paper_claim_active"])
        self.assertEqual(task["active_candidate_problem"], "unselected")
        self.assertEqual(task["primary_problem"], "unselected")

        p0 = load_nested_orbit_config(ROOT / audit["p0_v2_config"])
        self.assertEqual(len(validate_nested_orbit_config(p0)), 6)

        changed = copy.deepcopy(p0)
        changed["source"]["morphometry_table_resolution"] = "all_resolutions"
        with self.assertRaisesRegex(NestedOrbitP0V2Error, "source correction"):
            validate_nested_orbit_config(changed)

        changed = copy.deepcopy(p0)
        changed["execution"]["job_submitted"] = True
        with self.assertRaisesRegex(NestedOrbitP0V2Error, "execution"):
            validate_nested_orbit_config(changed)
        ...
        with self.assertRaisesRegex(ProtocolError, "AneuX reliability direction"):
            validate_protocol(candidate)

    def test_endovascular_collision_anticipation_batch_is_fail_closed(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        audit = problem[
            "endovascular_collision_anticipation_and_release_reappraisal"
        ]
        watch = problem["public_source_watch_v20"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(gate["current_batch_best_score"], 34.0)
        self.assertEqual(gate["current_batch_best_residual_novelty"], 2.5)
        self.assertEqual(
            audit["best_candidate_ids"],
            [
        ...
        self.assertEqual(audit["cathaction_paper_reported_videos"], 569)
        self.assertEqual(audit["archive_count"], 4)
        self.assertTrue(audit["human_segmentation_archive_present"])
        self.assertFalse(audit["human_collision_archive_present"])
        self.assertFalse(audit["chronological_collision_onset_contract_declared"])
        self.assertFalse(audit["procedure_specimen_anatomy_identifiers_declared"])
        self.assertFalse(audit["archive_payload_accessed"])
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertFalse(audit["scientific_server_queried"])
        self.assertFalse(audit["gpu_training_authorized"])
        self.assertFalse(audit["junjinyong_accessed"])
        self.assertEqual(watch["watch_count"], 33)

        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"][
            "endovascular_collision_anticipation_and_release_reappraisal"
        ]["p0_registered"] = True
        with self.assertRaisesRegex(ProtocolError, "collision-anticipation"):
            validate_protocol(candidate)

    def test_molecular_biomarker_and_treatment_outcome_batch_is_fail_closed(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        audit = problem[
        ...
        self.assertEqual(audit["pxd024615_cohort_i_samples"], 212)
        self.assertEqual(audit["pxd024615_cohort_ii_samples"], 32)
        self.assertEqual(audit["pxd013442_effective_discovery_biological_pools"], 4)
        self.assertEqual(audit["gse231922_samples"], 30)
        self.assertEqual(audit["treatment_source_patients"], 436)
        self.assertFalse(audit["pxd024615_public_3dra_image_release_identified"])
        self.assertFalse(audit["pxd024615_future_event_timeline_released"])
        self.assertFalse(audit["treatment_source_is_counterfactual_effect_model"])
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertFalse(audit["scientific_server_queried"])
        self.assertFalse(audit["gpu_training_authorized"])
        ...
        with self.assertRaisesRegex(
            ProtocolError, "molecular-biomarker/treatment-outcome"
        ):
            validate_protocol(candidate)

    def test_structured_vessel_and_embargoed_4dflow_batch_is_fail_closed(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        audit = problem["structured_vessel_and_embargoed_4dflow_reappraisal"]
        watch = problem["public_source_watch_v19"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(gate["current_batch_best_score"], 34.0)
        self.assertEqual(gate["current_batch_best_residual_novelty"], 2.5)
        self.assertEqual(
        ...
        self.assertEqual(audit["venet_exact_public_git_masks"], 20)
        self.assertFalse(audit["venet_public_source_mra_redistributed"])
        self.assertFalse(audit["rsna_patient_grouped_split_explicit"])
        self.assertTrue(audit["cmrx4dflow_embargo_after_submission_deadline"])
        self.assertEqual(audit["device_phantom_base_anatomies"], 1)
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertFalse(audit["scientific_server_queried"])
        self.assertFalse(audit["gpu_training_authorized"])
        self.assertFalse(audit["junjinyong_accessed"])
        self.assertEqual(watch["watch_count"], 32)

        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"][
            "structured_vessel_and_embargoed_4dflow_reappraisal"
        ]["p0_registered"] = True
        with self.assertRaisesRegex(ProtocolError, "structured-vessel/4D-flow"):
            validate_protocol(candidate)
    ...
    def test_top_level_problem_selection_cannot_select_method_or_gpu(self) -> None:
        ...
        with self.assertRaisesRegex(
            ProtocolError, "sole response-fidelity source lead"
        ):
            validate_protocol(candidate)

    def test_asah_segmentation_outcome_batch_is_rejected_before_payload_or_compute(self) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        audit = problem["aneurysmal_sah_segmentation_outcome_reappraisal"]
        self.assertEqual(self.protocol["schema_version"], "11.9")
        self.assertEqual(gate["current_batch_best_score"], 34.0)
        self.assertEqual(gate["current_batch_best_residual_novelty"], 2.5)
        self.assertEqual(audit["best_candidate_id"], "cross_aetiology_small_volume_asah_transport")
        self.assertEqual(audit["all_candidate_scores"], [29.0, 28.5, 28.0, 28.0, 27.0, 22.5])
        self.assertFalse(audit["zenodo_archive_opened"])
        self.assertFalse(audit["versioned_image_mask_outcome_join_public"])
        ...
        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"]["aneurysmal_sah_segmentation_outcome_reappraisal"][
    ...
    def test_source_watch_v14_is_exact_metadata_only_and_fail_closed(self) -> None:
        ...
        self.assertEqual(watch["watch_count"], 23)
        self.assertEqual(watch["asah_zenodo_revision"], 2)
        self.assertEqual(watch["asah_archive_bytes"], 648502298)
        ...
        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"]["public_source_watch_v14"][
    ...
    def test_culprit_mimic_batch_is_rejected_before_data_request_or_compute(
        self,
    ) -> None:
        problem = self.protocol["problem_selection"]
        gate = problem["future_source_admission_v2"]
        ...
        self.assertEqual(
            (
                audit["cta_culprit_total_patients"],
                audit["cta_culprit_total_aneurysms"],
                audit["cta_culprit_total_hospitals"],
            ),
            (272, 607, 8),
        )
        self.assertEqual(
            (
                audit["vwi_symptomatic_patients"],
                audit["vwi_symptomatic_aneurysms"],
                audit["vwi_symptomatic_aneurysms_positive"],
                audit["vwi_symptomatic_aneurysms_negative"],
            ),
            (30, 82, 30, 52),
        )
        self.assertEqual(
            (
                audit["smaller_counterpart_patients"],
                audit["smaller_counterpart_largest_ruptured_patients"],
                audit["smaller_counterpart_smaller_ruptured_patients"],
            ),
            (285, 261, 24),
        )
        self.assertEqual(
            (
                audit["infundibulum_total_outpouchings"],
                audit["infundibulum_unequivocal_count"],
                audit["infundibulum_conundrum_count"],
                audit["infundibulum_dsa_rereview_count"],
            ),
            (665, 321, 344, 10),
        )
        self.assertTrue(audit["ican_public_table_is_simulated_not_patient_evidence"])
        self.assertFalse(
            audit[
    ...
    def test_topbrain_rsna_release_batch_is_rejected_before_terms_or_compute(
        ...
        self.assertEqual(
            (
                audit["topbrain2025_public_volumes"],
                audit["topbrain2025_public_same_patient_cta_mra_pairs"],
                audit["topbrain2025_public_independent_patients"],
            ),
            (50, 25, 25),
        )
        self.assertEqual(
            (
                audit["topbrain2025_cta_label_count"],
                audit["topbrain2025_mra_label_count"],
                audit["topbrain2025_overlapping_cta_mra_label_count"],
            ),
            (40, 42, 34),
        )
        self.assertTrue(
        ...
        self.assertFalse(audit["topbrain2025_terms_accepted"])
        self.assertFalse(audit["topbrain2025_payload_accessed"])
        self.assertEqual(audit["topcow_unique_patients"], 200)
        self.assertEqual(audit["topcow_external_largeia_aneurysm_patients"], 12)
        self.assertEqual(audit["bravecowcow_pseudomask_training_series"], 4348)
        self.assertFalse(
            audit[
        ...
        ]["topbrain2025_public_independent_patients"] = 50
        with self.assertRaisesRegex(ProtocolError, "TopBrain 2025 and BraveCoWCoW"):
            validate_protocol(candidate)
    ...
    def test_source_watch_v12_is_exact_metadata_only_and_fail_closed(self) -> None:
        watch = self.protocol["problem_selection"]["public_source_watch_v12"]
        self.assertEqual(watch["config"], "configs/source_watch_v12.json")
        self.assertEqual(
            watch["extends_historical_config"], "configs/source_watch_v11.json"
        )
        self.assertEqual(watch["watch_count"], 18)
        self.assertEqual(
        ...
        candidate = copy.deepcopy(self.protocol)
        candidate["problem_selection"]["public_source_watch_v12"][
    ...
    def test_vmr_growth_paired_p0_closes_execution_incomplete_without_repair(self) -> None:
        ...
        self.assertFalse(audit["junjinyong_accessed"])
        self.assertEqual(
            task["active_candidate_problem"],
            "unselected",
        )
        self.assertEqual(
            task["candidate_primary_estimand"],
            "unselected",
        )
        self.assertEqual(
            task["candidate_secondary_estimand"],
            "unselected",
        )
        self.assertEqual(
    ...
    def test_topaneu_registered_design_is_separated_from_realized_release(self) -> None:
        audit = self.protocol["problem_selection"][
        ...
        self.assertEqual(audit["registered_task1_train_volumes"], 500)
        self.assertEqual(audit["registered_private_test_volumes"], 350)
        self.assertEqual(audit["realized_training_scans"], 417)
        self.assertEqual(audit["realized_unique_patients"], 409)
        self.assertEqual(audit["registered_train_gold_vessel_masks"], 50)
        self.assertFalse(
            audit["realized_casewise_gold_silver_indicator_publicly_exposed"]
        )
        self.assertTrue(
            audit[
        ...
        self.assertFalse(audit["zenodo_license_applies_to_medical_release"])
        self.assertFalse(audit["user_terms_acceptance_verified"])
        self.assertFalse(audit["medical_image_mask_json_or_annotation_payload_accessed"])
        self.assertFalse(audit["surface_vector_reactivated"])
        self.assertFalse(audit["surface_vector_closed_job_repaired_or_rerun"])
        self.assertFalse(audit["p0_registered"])
        self.assertFalse(audit["method_selected"])
        self.assertFalse(audit["scientific_server_queried"])
        self.assertFalse(audit["gpu_training_authorized"])
        ...
        with self.assertRaisesRegex(ProtocolError, "TopAneu registered design"):
            validate_protocol(candidate)
    ...
    def test_i0a_pass_cannot_select_a_method_or_call_cfd_mri_truth(self) -> None:
        ...
        with self.assertRaisesRegex(ProtocolError, "source-rejected"):
            validate_protocol(candidate)
        candidate = copy.deepcopy(self.protocol)
