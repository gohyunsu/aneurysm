...
> [`exact P0`](configs/aneumo_lineage_p0.json) admitted one metadata-only
> `introai9` CPU/PBS job. Exact source `d3eb3d3…`, job `115386.ECE-util1`
> finished at walltime 20:36 with exit `-29` before the first small source
> completed. Cache/result JSON/raw log are 0 and all scientific checks are
> unevaluated. The [`execution record`](results/aneumo_lineage_p0_execution_20260810.json)
> closes this version without repair, rerun, P1, method, architecture, GPU or
> outer test. `junjinyong` remains excluded from every AURORA operation.
...
고치는 것 자체는 novelty가 아니며 GNN, neural operator, Transformer를 포함한
모델과 GPU는 선택하지 않았습니다.
...
public source `754ed746…`의 `introai9` P0가 payload 전에 exit 28로 끝나
`execution-incomplete/no scientific verdict`로 닫혔습니다. Dependency/reader/
transport repair, same-contract rerun과 P1은 열지 않습니다. 상세 이력은
[`cycle-functional WSS audit`](docs/cycle-functional-wss-audit-2026-08-09.md)과
...
아니라 13-class Circle-of-Willis 혈관 해부구조이고, aneurysm supervision은
center point와 presence/territory label입니다. 공식 voxel aneurysm mask가
선택적으로 존재한다는 전제가 틀렸으므로 access를 받아도 원래 estimand는
성립하지 않습니다. 근거와 정확한 기각 논리는
[`RSNA supervision-semantics audit`](docs/rsna-supervision-semantics-audit-2026-08-09.md)에
있습니다.

공식 1차 자료만 사용한 CADA, ADAM, IntrA와 TopCoW 비교도 이 후보를
구제하지 않습니다. 이들은 향후 fully supervised external control 또는
vascular-anatomy pretraining에는 쓸 수 있지만, 기각된 annotation-selection
...
있는지를 물었습니다. Generic super-resolution, denoising, PINN
