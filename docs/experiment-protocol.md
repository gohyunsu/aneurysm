...
> anatomy. No P0/P1, data transfer, method, architecture, scientific-server
> query or PBS/GPU job is registered. Any future version requires a new usable
> patient-level aneurysm 4D-flow mask/velocity/WSS asset and an independent
> estimand before compute.
...
> aSAH, VWI, synthetic-DSA or other patient payload was opened. No executable P0,
> PBS/GPU job, method, architecture or outer test is authorized. A material new
> source must first score at least 32, after which only a separately frozen,
> method-free CPU/read-only P0 may run through PBS on `introai9`. `junjinyong`
> remains excluded from connection, query, transfer, submission and monitoring.
...
> 종료돼 mapping/morphometry/split/license/LFS check가 모두 미평가다. Archive
