...
> V2는 모델별 세 checkpoint를 중복 없이 배정하고, 두 learned member 각각이
> power-law field control보다 2% 이상 열등하지 않음을 one-sided 95% upper
> bound로 요구한다. 이 competence gate가 없으면 낮은 response error가 단순한
> underfit/over-smoothing 산물일 수 있다. V2도 비활성이고 model code/GPU는 0이다.

> **Schema 11.8 P1 baseline boundary:** P1의
> historical [v1 설계 템플릿](../configs/aneumo_response_fidelity_p1_template_v1.json)은 proposal
...
> 만드는지는 scientific gate에서 미평가다. Split correction, group DRO,
> contrastive invariance, geometry-informed/shape-derivative operator는 단독
> novelty로 올리지 않는다. Active shortlist와 GPU authorization은 0이다.
...
> inverse-intensity weighting과 treatment token은 모두 plausible component지만
> ImageFlowNet, longitudinal diffusion과 TESAR-CDE의 direct prior이며 62 patient/
> 9 DCI로 잔여 claim을 검증할 수 없다. 따라서 현재 GNN을 포함한 headline
> architecture와 GPU authorization은 **미선정/0**이다.
> [`source audit`](longitudinal-perfusion-source-audit-2026-08-10.md)
...
> 따라서 현재 headline architecture는 없다.
...
> 상세 판정은 [`PINN rupture-status direct-prior audit`](pinn-rupture-direct-prior-audit-2026-08-10.md)을 따른다.
...
FSI–wall best 31.0/40 and all source-rejected · longitudinal-perfusion best
31.0/40 and all source-rejected · active shortlist/selected primary
...
prediction, temporal residual projection과 Graph Transformer는 구현된 현재
...
LesionDETR류 query matching과 aneurysm topology/shape loss는 모두 direct prior다.
P0가 통과하지도 scientific fail을 내지도 못했으므로 P1이나 architecture를
열지 않는다.

Parser를 보강한 P0r이나 같은 데이터의 local repair는 등록 계약상 만들지 않는다.
