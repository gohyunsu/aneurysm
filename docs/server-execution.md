...
> or monitored. Future gate-authorized execution remains `introai9` PBS only;
> login-node GPU is prohibited. Never access, query, transfer to, submit to or
> monitor `junjinyong`.

> **Schema 10.1 server boundary · 2026-08-12:** This reappraisal used public
> paper and repository metadata only. No scientific server was queried, no
> medical/simulation payload was transferred and no PBS/GPU job was created or
> monitored. Future gate-authorized execution remains `introai9` PBS only;
> login-node GPU is prohibited. Never access, query, transfer to, submit to or
> monitor `junjinyong`.

> **Schema 10.0 server boundary · 2026-08-12:** This update used public
> literature, official challenge metadata and terms only. No scientific server
> was queried, no data was transferred and no PBS/GPU job was created or
> monitored. Future gate-authorized execution remains `introai9` PBS only;
> login-node GPU is prohibited. Never access, query, transfer to, submit to or
> monitor `junjinyong`.
...
> use only `introai9` PBS; login-node GPU commands are prohibited. Never connect
...
> payload was transferred and no PBS/GPU job was created. The last actual
> `introai9` observation was not refreshed. `junjinyong` was not accessed and
> remains prohibited; login-node GPU commands remain prohibited.
