...
> Same-lesion preprocessing reliability remains a sound evaluation principle,
> not a paper identity. P0 v1/v2 are frozen and unexecuted; no v3, active lead,
> method, architecture, compute, outer test or manuscript claim exists. The next
> permitted step is a fresh problem-level audit over documented holdings.
...
> 101 fluid mesh를 paired rigid/FSI time-series label로 혼동하지 않는다. Animal
> inverse mechanics, five-aneurysm micro-CT wall thickness와 one-device showcase는
> 독립 단위와 target linkage가 부족하고 generic FSI operator·residual learning·
> conformal referral은 direct prior다. 여섯 후보는
> 30.5/29.5/26.5/24.5/29.0/31.0으로 모두 기각한다. 자세한 논리는
> [`frozen audit`](fsi-wall-source-audit-2026-08-10.md)에 있다. Payload/P0/model/
> GPU는 0이고 향후 compute도 `introai9` PBS만 허용한다.
...
independent device-response unit가 없고 generic FSI/conformal operator는 direct
prior다. 여섯 후보 모두 32 미만이며 payload, P0, method, architecture와 GPU는
0이다.

직전 기준선은
[`acquisition-flow-source-audit-2026-08-10.md`](acquisition-flow-source-audit-2026-08-10.md)이고
최고점 27.5/40으로 기각됐다.
...
primary estimand 후보로 두었다. Score는 34/40이었지만 selected primary는 아니었다.
...
그마저 contribution으로 쓰지 않는다. Exact P0가 task gate 전에
execution-incomplete로 닫혔으므로 이 가설은 현재 inactive다.
...
같은 source 재제출이 없으며, pass도 method-free P1 등록만 허용하도록 했다.
실제 exact `42cc3c7…` job `115177`은 exit 2, walltime 37분 뒤 initial tabular
transport attempt exhaustion으로 종료됐다. Complete/partial tabular archive와
CSV parse는 0이고 model HEAD/range/central-directory/member에도 도달하지 않아
13개 check는 전부 미평가다. Repair/rerun/P1 없이 candidate를 닫았다. 근거와
kill rule은
...
morphometric feature가 보고된다. 그러나 exact P0는 첫 tabular archive completion
전에 transport attempt를 소진해 row-level CSV를 parse하지 않았고 model central
directory와 mesh payload에도 도달하지 않았다. Scientific gate, task unit,
selected primary, architecture와 성능 증거는 0이다. Same-source repair/rerun과
P1은 금지하며 다음 허용 작업은 fresh problem-level source/asset audit뿐이다.
